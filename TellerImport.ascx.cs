...
namespace RockWeb.Plugins.Finance
{
    /// <summary>
    ...
    public partial class TellerImport : Rock.Web.UI.RockBlock
    {
        ...
        private Guid? _batchNameDtGuid = null;
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            _batchNameDtGuid = GetAttributeValue( "BatchNames" ).AsGuidOrNull();
            ...
            if ( !Page.IsPostBack )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    ddlBatch.DataBind();
                    ddlBatch.Items.Insert( 0, new ListItem( "[New Batch]", "" ) );

                    if ( _batchNameDtGuid.HasValue )
                    {
                        var dt = DefinedTypeCache.Read( _batchNameDtGuid.Value );
                        if ( dt != null )
                        {
        ...
        protected void btnConfirm_Click( object sender, EventArgs e )
        {
            ...
            string accountMatchField = GetAttributeValue( "AccountMatchType" );
            ...
            foreach ( var node in xml.Root.Descendants().Where( n => n.Name == "Gift" ) )
            {
                ...
                var status = ProcessTransaction( node, batchId, attributeId, binaryFileTypeId, accountMatchField, out errorMessages );
                ...
                else
                {
                    if ( _batchNameDtGuid.HasValue )
                    {
                        ...
                        if ( dvId.HasValue )
                        {
                            ...
                            if ( dv != null )
                            {
                                batchName = string.Format( "a new batch (<strong><em>{0}</em></strong>)", dv.Value );
                            }
                        }
                    }
                }
